...
<iframe style="display:none;" name="back" id="back"></iframe>
    ...
    for($i=$start;$i<$start+5;$i++){
      if($i<$end){
?>
	<li><?=mb_substr($newsArr[$i]['name'],0,10)?><div class='all' style='display:none;'><?=$newsArr[$i]['name']?></div></li>
<?php
	  }
	}
  $totalp=ceil($end/5);
  $last=$p-1;
  $next=$p+1;
  ?><a href="?redo=news.php&p=<?=$last>0?$last:1;?>" style="font-size:30px;"><</a><?php
  for($i=1;$i<=$totalp;$i++){
    if($i==$p){
      ?><a href="?redo=news.php&p=<?=$i?>" style="font-size:30px;"><?=$i?></a><?php
    }else{
      ?><a href="?redo=news.php&p=<?=$i?>" style="font-size:20px;"><?=$i?></a><?php
    }    
  }
  ?><a href="?redo=news.php&p=<?=$next<=$totalp?$next:$totalp;?>" style="font-size:30px;">></a><?php
?>
